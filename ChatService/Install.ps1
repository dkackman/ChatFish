...
python -m pip install diffusers[torch] transformers accelerate scipy ftfy safetensors
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
...
Write-Output ""
Write-Output "chat fish worker installation is now complete."
Write-Output ""
Write-Output "Type '.\venv\scripts\activate' and then 'uvicorn main:app --reload' to begin."
